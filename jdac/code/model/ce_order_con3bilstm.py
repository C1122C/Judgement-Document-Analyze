...
class CNN(object):
    ...
    def conv(self, x_word, y_word, input_x, input_y, x_bi, y_bi):
        with tf.name_scope("conv_c3"):
            ...
            # op1[128,1,256] con30fact
            #op_con30fact = tf.reduce_max(conv1, reduction_indices=[1], name='gmp1', keepdims=True)
            ...
            conv2 = tf.layers.conv1d(input_y, filters=self.config.FILTERS, kernel_size=self.config.KERNEL_SIZE,
                                     name='conv4_3')
            # op2[128,1,256] con30law
            #op_con30law = tf.reduce_max(conv2, reduction_indices=[1], name='gmp2', keepdims=True)
            conv3 = tf.layers.conv1d(tf.tile(input=tf.expand_dims(x_bi, axis=1), multiples=[1, 30, 1]),
                                     filters=self.config.FILTERS, kernel_size=self.config.KERNEL_SIZE,
                                     name='conv5_3')
            # op3[128,1,256] bi_fact
            #op_bi_fact = tf.reduce_max(conv3, reduction_indices=[1], name='gmp3', keepdims=True)
            conv4 = tf.layers.conv1d(tf.tile(input=tf.expand_dims(y_bi, axis=1), multiples=[1, 30, 1]),
                                     filters=self.config.FILTERS, kernel_size=self.config.KERNEL_SIZE,
                                     name='conv6_3')
            # op4[128,1,256] bi_law
            #op_bi_law = tf.reduce_max(conv4, reduction_indices=[1], name='gmp4', keepdims=True)
            conv5 = tf.layers.conv1d(x_word, filters=self.config.FILTERS, kernel_size=self.config.KERNEL_SIZE,
                                     name='conv7_3')
            # op5[128,1,256] word_fact
            #op_word_fact = tf.reduce_max(conv5, reduction_indices=[1], name='gmp5', keepdims=True)
            conv6 = tf.layers.conv1d(y_word, filters=self.config.FILTERS, kernel_size=self.config.KERNEL_SIZE,
                                     name='conv8_3')
            # op6[128,1,256] word_law
            #op_word_law = tf.reduce_max(conv6, reduction_indices=[1], name='gmp6', keepdims=True)
            # shape[128,26*3,256]
            fact_all = tf.concat([conv5, conv1, conv3], axis=1)
            law_all = tf.concat([conv6, conv2, conv4], axis=1)
            # shape[128,26*3,256]
            conv7 = tf.layers.conv1d(fact_all, filters=self.config.FILTERS, kernel_size=self.config.KERNEL_SIZE,
                                     name='conv9_3')
            # shape[128,252,256]
            op1 = tf.reduce_max(conv7, reduction_indices=[1], name='gmp7')
            conv8 = tf.layers.conv1d(law_all, filters=self.config.FILTERS, kernel_size=self.config.KERNEL_SIZE,
                                     name='conv10_3')

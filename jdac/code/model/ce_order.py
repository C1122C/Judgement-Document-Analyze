...
class CNN(object):
    ...
    def gate1(self, ks, inputx):
        with tf.name_scope("gate"):
            ...
            k_1_init, k_2_init, k_3_init = ks[:, 0, :], ks[:, 1, :], ks[:, 2, :]  # [None,d]切片
            k_1 = tf.reshape(tf.keras.backend.repeat_elements(k_1_init, rep=self.config.FACT_LEN, axis=1),#重复张量元素
                             shape=[-1, self.config.FACT_LEN, 1, self.config.EMBDDING_DIM])#参数变化成shape的形式
            ...
            k_3 = tf.reshape(tf.keras.backend.repeat_elements(k_3_init, rep=self.config.FACT_LEN, axis=1),
                             shape=[-1, self.config.FACT_LEN, 1, self.config.EMBDDING_DIM])
            inputx_epd = tf.expand_dims(inputx, axis=2) #[b,l,1,d]增维
